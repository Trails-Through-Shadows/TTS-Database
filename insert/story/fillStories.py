...
# Open the JSON file
dataFilePath = currentFolderPath + "/stories.json"
with open(dataFilePath, encoding='utf8') as dataFile:
    data = json.load(dataFile)

# Write the SQL file
sqlFilePath = currentFolderPath + "/5-stories.sql"
...
for story in data:
    ...
    # Params
    storyIDcampaignLocation = story["idCampaignLocation"]
    storyTrigger = story["trigger"]
    story = story["story"]

    sqlFile.write("-- Story {}\n".format(storyID))
    sqlFile.write(
        "INSERT INTO Story (id, idCampaignLocation, `trigger`, story) "
        "VALUES ({}, {}, '{}', '{}');\n"
        .format(storyID, storyIDcampaignLocation, storyTrigger, story)
    )
